...
export const CustomModal = forwardRef((props, ref) => {
  ...
  const handleModalSaveChanges = async () => {
    ...
    const newReview = Object.assign({}, modal.current.save());
  ...
  const validateFields = (review) => {
    ...
    if (review.authorScore) {
      const score = parseInt(review?.authorScore);
        ...
        <Modal.Body>
          <Container>
            {params.displayEditForm && (
              <ReviewCreateBody review={editedReview} modeCreate={false} ref={modal} />
            )}
            {params.displayCreateForm && (
              <ReviewCreateBody modeCreate={true} review={emptyReview} ref={modal} />
            )}
