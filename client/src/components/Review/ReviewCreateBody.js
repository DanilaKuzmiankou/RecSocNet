...
export const ReviewCreateBody = forwardRef((props, ref) => {
  ...
  const currentReview = Object.assign({}, props.review);
  ...
  function onMultiselectSelect(selectedList, selectedItem) {
    if (selectedItem && selectedTags.indexOf(selectedItem) === -1 && selectedItem.trim()) {
      const newTagsList = [...selectedTags, selectedItem];
  ...
  return (
    <div>
      <Form>
        ...
        <UploadImage updateImages={updateImages} filesUrl={currentReview?.images} />
      </Form>
    </div>
  );
});
